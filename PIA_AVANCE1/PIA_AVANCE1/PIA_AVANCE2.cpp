...
using namespace std;          //Solo falta acomodar algo en los Edit Control de Modificar Carnet
...
wchar_t* fExpBMP(HWND hwnd, wchar_t* ubicacion);                //referente a abrir y guardar foto 
void fCBMP2(HWND hwnd, wchar_t* ubicacion);                      //referente a cargar foto
void fEscribirC();
void fLeerC();
void fEscribirP();
void fLeerP();
...
};              //este es el que modificare para hacer listas doblemente ligadas
...
struct ARBOLCARNET {
    ...
    }           //esta funcion es la que se usa para borrar un nodo del arbol
...
}//se debe poner luego de eliminar a cualquier vacuna o persona
            ...
            if (auxiliar != NULL) {
                while (salir != true) {
                    ...
                    if ((auxiliar->usuario->usuario == us) && (auxiliar->usuario->contrasena == con)) {
                        MessageBox(hwnd, L"INICIO DE SESION CORRECTO", L"CORRECTO", MB_ICONEXCLAMATION);
                        CASDialog(hwnd, IDD_RP, fRP);
                    }
...
INT_PTR CALLBACK fRP(HWND hwnd, UINT uMsg, WPARAM wparam, LPARAM lparam) {
    ...
    case WM_COMMAND: {
        ...
        case IDOK: {
            ...
            DateTime_GetSystemtime(hDATIP, &fecha);                          //EN LA 1512 DE PIA_PA PUEDES VER COMO PONER EN UN STRING ESTO
            GetWindowText(hCALLE, CALLE, 100);
            GetWindowText(hCOL, COL, 100);
            GetWindowText(hCIU, CIU, 100);
            GetWindowText(hEST, EST, 100);
            GetWindowText(hESC, ESC, 100);              //en proceso
            GetWindowText(hTEL, TEL, 100);
            GetWindowText(hS, S, 100);                  //en proceso
...
INT_PTR CALLBACK fRC(HWND hwnd, UINT uMsg, WPARAM wparam, LPARAM lparam) {
    ...
    case WM_INITDIALOG: {
        HMENU hMenuP = LoadMenu(GlobalhInstance, MAKEINTRESOURCE(IDR_MENU1));
        SetMenu(hwnd, hMenuP);
        HWND hlbPer = GetDlgItem(hwnd, IDC_LIST);     //referente al listbox de personas
        if (busqueda.size() != 0) {
            NODOSIMPLEP* auxiliar;
            auxiliar = LISTA_P.ORIGENP;
            while (auxiliar != NULL) {
        ...
        if (fotov2 == true) {                      //carga la foto cuando se selecciona a alguien
            fCBMP2(hwnd, auxiliarfoto);
...
INT_PTR CALLBACK fEV(HWND hwnd, UINT uMsg, WPARAM wparam, LPARAM lparam) {
    ...
    case WM_INITDIALOG: {
        ...
        if (listabv == true) {
            ...
            string descripcion;
            descripcion = auxiliarv->descripcion; //aqui ya puedes hacer el append y unir todo en un solo string
        ...
        case IDOK:{
            if (bvac.size() != 0) {
                if (LISTA_C.ORIGEN != NULL) {
                    NODOSIMPLEC* aux;
                    aux = LISTA_C.ORIGEN;
                    while ((aux->carnet->clabev)!=bvac){          //Primero lo quitamos del arbol
                        aux=aux->siguiente;
                    }
                    fEliminarNAV(aux->carnet);
                    aux = LISTA_C.ORIGEN;
                    while (aux != NULL) {                        //Luego eliminamos el carnet
...
INT_PTR CALLBACK fEC(HWND hwnd, UINT uMsg, WPARAM wparam, LPARAM lparam) {
    ...
    case WM_INITDIALOG: {
        ...
        SetMenu(hwnd, hMenuP);
        hBusqueda = GetDlgItem(hwnd, IDC_P);
        if (auxiliarc4!=NULL) {                      //carga la foto cuando se selecciona a alguien
            fCBMP2(hwnd, fStoW(auxiliarc4->nodopersona->DirFoto));
            hInfo = GetDlgItem(hwnd, IDC_INFO);      //Estan al reves pero funcionan
    ...
    case WM_COMMAND:
        ...
        case IDC_BCB:
        {
            ...
            if (BCAR[0] != 0) {
                ...
                if (validacion == true) {
                    int numerobus = stoi(fCharToS(BCAR));                     //BUSQUEDA BINARIA
...
INT_PTR CALLBACK fMC(HWND hwnd, UINT uMsg, WPARAM wparam, LPARAM lparam) {
    ...
    case WM_INITDIALOG: {
        ...
        if (fotov3 == true) {                      //carga la foto cuando se selecciona a alguien
            EnableWindow(hBusqueda2, TRUE);                        //deja buscar solo luego de buscar el carnet por numero
            
            fCBMP2(hwnd, fStoW(auxiliarc2->nodopersona->DirFoto));
            hInfo = GetDlgItem(hwnd, IDC_INFO2);      //Estan al reves pero funcionan
    ...
    case WM_COMMAND:
        ...
        case IDC_BCB:
        {
            ...
            if (BCAR[0] != 0) {
                ...
                if (validacion == true) {                                    //BUSQUEDA BINARIA
...
void fLeerP() {   //No olvidar que al leer se crea una nueva persona, pero su direccion en memoria se borra al salir, por eso llamé al metodo constructor de esa manera
...
bool fComprobarS(string comprobar) {
    for (int i = 0; i < comprobar.length(); i++) {
        if (isspace(comprobar[i])){
            return false;                          //regresa false cuando hay espacio
        }
    }
    return true;                                   //regresa true cuando no hay espacios
}
...
wchar_t* fExpBMP(HWND hwnd, wchar_t* ubicacion) {              //fotos
...
void fCBMP2(HWND hwnd, wchar_t* ubicacion) {                       ///codigo de las fotos
    HBITMAP hBitmap = (HBITMAP)LoadImage(
        NULL,
        ubicacion,
        IMAGE_BITMAP,
...
void fListaVacuna(HWND hwnd) {
    HWND hlbVac = GetDlgItem(hwnd, IDC_VAC);   //referente al listbox de vacunas
    if (LISTA_S.ORIGEN != NULL) {
        NODOSIMPLE* auxiliar;
        auxiliar = LISTA_S.ORIGEN;
        while (auxiliar != NULL) {
